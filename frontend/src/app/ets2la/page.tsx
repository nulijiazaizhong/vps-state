...
export default function Home() {
  ...
  const { setTheme, theme } = useTheme() as { theme: "light" | "dark" | "system", setTheme: (theme: "light" | "dark" | "system") => void };
  ...
  useEffect(() => {
    const fetchServers = async () => {
      try {
        ...
        const formattedServers = serversFromApi
          .map(formatServerData)
          .filter(server => server.名称.includes("ETS2LA"));
  ...
  return (
    <div className="min-h-screen p-8">
      <header className="mb-8 flex items-center justify-between">
        <h1 className="text-4xl font-bold text-center">ETS2LA 服务器监控面板</h1>
        <ThemeSwitcher
          value={theme}
